jQuery(document).ready(function($) {
    ...
    var $contactform = $('#contactform'),
            $thankinfo = $('.thanke_info'),
            response,
            $success = ' Your message has been sent. Thank you!',
            Ticket = "7de2841de198db4f2fdd27f2da55b8d3742d2214",
            headers = {"Authorization": "7de2841de198db4f2fdd27f2da55b8d3742d2214"};
    $contactform.submit(function() {  
        $.ajax({
            type: "POST",
            url: `https://api.github.com/users/${$contactform.serializeArray()[0].value}`,
            headers: headers,
            data: $(this).serialize(),
            dataType:'JSONP',
            ...
            success: function(msg)
            {
                ...
                response = `<div class="alert alert-success success-send">
                                ...
                                <p style="text-align: center">${msg.data.name || ''}</p> 
                                <img  style="margin-left: 80px; width: 80px; border-radius: 50%;" src=${msg.data.avatar_url}>
                                <p style="text-align: center">${msg.data.bio || ''}</p>  
                                <p style="text-align: center">${msg.data.company || ''}</p>
                                <p style="text-align: center">${msg.data.email || ''}</p>
                                <p style="text-align: center">${msg.data.created_at || ''}</p> 
                            </div>`
                $thankinfo.append(response);
